...
module "s3_bucket_electricity_prices" {
  source = "terraform-aws-modules/s3-bucket/aws"

  bucket        = "electricity-prices-staging"
  acl           = "private"
  force_destroy = true

  attach_policy = true
  policy        = data.aws_iam_policy_document.bucket_policy_electricity_prices.json
...
data "aws_iam_policy_document" "bucket_policy_electricity_prices" {
  statement {
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::315911064428:user/s3-upload-role"]
    ...
    resources = [
      "arn:aws:s3:::electricity-prices-staging/*",
    ]
  }
}
